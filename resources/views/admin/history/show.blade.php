  ...
  <div class="card bg-base-100 shadow-md">
    <div class="lg:flex ">
      <div class="lg:w-1/3 p-4">
        @php
          $imagePath = public_path('images/events/' . ($order->event?->gambar ?? ''));
        @endphp
        <div style="display: flex; justify-content: center;">
          @if (!empty($order->event?->gambar) && file_exists($imagePath))
            <img src="{{ asset('images/events/' . $order->event->gambar) }}" alt="{{ $order->event?->judul ?? 'Event' }}" style="max-width: 300px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
          @else
            <img src="{{ asset('assets/images/konser.jpg') }}" alt="Gambar Event Default" style="max-width: 300px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
          @endif
        </div>
