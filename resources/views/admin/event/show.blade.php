    ...
    <div class="container mx-auto p-10">
        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-body">
                ...
                <form id="eventForm" class="space-y-4" method="post"
                    ...
                    <div class="form-control">
                        ...
                        <input type="datetime-local" name="tanggal_waktu" class="input input-bordered w-full"
                            value="{{ $event->tanggal_waktu ? $event->tanggal_waktu->format('Y-m-d\TH:i') : '' }}" disabled required />
                    </div>
                ...
                <!-- Preview Gambar -->
                @php
                    $imagePath = public_path('images/events/' . $event->gambar);
                @endphp
                @if (!empty($event->gambar) && file_exists($imagePath))
                    <div style="display: flex; justify-content: center;">
                        <img src="{{ asset('images/events/' . $event->gambar) }}" alt="Gambar Event" style="max-width: 300px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
                    </div>
                @else
                    <img src="{{ asset('assets/images/konser.jpg') }}" alt="Gambar Event Default">
                @endif
            </form>
        </div>
    </div>
